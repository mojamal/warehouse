...
ITEMS_LIST = HOME / "ebayitems.txt"        # 77 items
INVENTORY = DOWNLOADS / DATE / "Inventory.txt"
LOGFILE = RUN_HOME / "logs" / f"get_ebay_count_{LONG_DATE}.log"
INVENTORY_COUNT = RUN_HOME / f"InventoryEbayCount_{DATE}.txt"

INVENTORY_SUM = RUN_HOME / "InventoryEbaySum.txt"
INVENTORY_TOTAL = RUN_HOME / "InventoryEbayTotal.txt"
...
# Begin processing
log(f"Begin Ebay product count aggregation {datetime.now()}")

# Truncate InventoryEbayCount file
...
log(f"End Ebay product count aggregation {datetime.now()}")

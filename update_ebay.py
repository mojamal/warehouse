...
MASTER_LIST = HOME / "EBAY_Warehouse.csv"
MASTER_NEW = RUN_HOME / f"EBAY_Warehouse.{DATE}.csv"

LOG_DIR = RUN_HOME / "logs"
LOGFILE = LOG_DIR / f"update_warehouse_ebay_{DATE}.log"

TOTALS_LIST = RUN_HOME / "InventoryEbayTotal.txt"
INVENTORY_REPORT = RUN_HOME / f"updatewhse-report-ebay.{DATE}.txt"

TEMPFILE = RUN_HOME / "tempfile.ebay"
TEMPFILE_ALL = RUN_HOME / "tempfile.ebay.all"
...
log(f"Begin updating inventory count for ebay items {datetime.now()}")
...
log("ensure all items in InventoryEbayTotal exist in the csv file")
...
if not BAD_CSV_FLAG:
    ...
    shutil.copy2(MASTER_LIST, OLD_CSV / f"EBAY_Warehouse.{LONG_DATE}.csv")
...
log(f"End updating inventory count for ebay items {datetime.now()}")
